...
function SpillContent() {
  ...
  const handleNavBack = () => {
    if (view === "picker")    return router.push("/activities")
  ...
  return (
    <>
      ...
      {/* ── Fixed back button — lives OUTSIDE the scale wrapper and every scroll
          container so it's always reachable no matter how far the user scrolls.
          position:fixed is relative to the viewport, not any parent transform,
          so the desktop pageScale transform doesn't affect it at all.
          zIndex:200 puts it above all page content and animations. ── */}
      <button
      ...
      {/* paddingTop:80 reserves space so no content hides behind the fixed button */}
      <div style={{
        fontFamily: "var(--font-dm-sans), sans-serif",
        ...
        <div style={pageScale < 1 ? {
          ...
          {/* SpillNav removed — back button is now fixed above (see above) */}
